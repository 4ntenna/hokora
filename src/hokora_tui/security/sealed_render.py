...
def body_for_render(msg: dict[str, Any], sealed_keys) -> str:
    ...
    Resolution order: ``msg["body"]`` if set; else AES-256-GCM decrypt
    of ``msg["encrypted_body"]`` keyed on channel_id; else empty for
    deleted-stub rows. Decrypt failures (missing key, wrong epoch,
    integrity mismatch) log WARNING and return ``DECRYPT_FAILED_MARKER``.
    """
    body = msg.get("body")
