...
class NodeItem(urwid.WidgetWrap):
    """Selectable two-row item for a discovered node.
    ...
    ``?h`` on path-table miss. Hops are queried live from
    ``RNS.Transport.path_table`` per announce. Star renders only when
    the node is bookmarked.
    """
