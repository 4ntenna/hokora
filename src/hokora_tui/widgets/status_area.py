...
Two-tier middle line: persistent ``set_context`` text (channel
breadcrumb / connection state / refresh-job updates) and transient
``set_notice`` overlays. Notices win for their TTL, then context
re-renders so the user sees current state, not stale text. A second
``set_notice`` replaces immediately rather than queueing.

Level styling is text-only — ``warn`` / ``error`` notices get a
``[warn]`` / ``[error]`` ASCII prefix; no emoji glyphs, matching the
daemon's plain-text log idiom.
"""
