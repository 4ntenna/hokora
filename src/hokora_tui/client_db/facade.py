...
Direct namespaced access is available via ``db.messages``, ``db.channels``, etc.
"""
...
class ClientDB:
    ...
    def __init__(
        ...
        *,
        ...
        ``encrypt=True`` (default) opens via SQLCipher; pre-encryption
        plaintext DBs are migrated once before open. ``encrypt=False``
        is the test-only escape hatch. ``notice`` receives migration
        progress strings when supplied.
        """
    ...
    def transaction(self) -> Iterator["ClientDB"]:
        """Run cross-store writes atomically; commits on exit, rolls back on exception.

        Yields the facade for ``tx.messages`` / ``tx.cursors`` access.
        Not reentrant — nested calls deadlock on the shared lock,
        which catches the design error explicitly.
        """
