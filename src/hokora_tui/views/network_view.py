...
The TUI mutates its own RNS config file directly via
:mod:`hokora.security.rns_config` (works in every topology because the
TUI always has filesystem access to its own config). Apply behaviour is
topology-aware: daemon-attached restarts via ``hokora seed apply
--restart``; standalone or remote-daemon TUIs require a TUI restart.
"""
...
class NetworkView:
    ...
    Layout::
        ...
        Pile [
            LineBox("Connection")      [status + uptime + link count + disconnect]
            LineBox("Seed Nodes")      [seed list + add edit + add button]
            LineBox("Announce Config") [auto-announce toggle + interval + announce now]
        ]
    """
