...
class FederationHandshakeOrchestrator:
    """Drives the 3-step challenge/response handshake + FS epoch-handshake init.

    Stateless between handshakes; per-mirror state lives on
    ``ChannelMirror``. ``mirrors_view``/``pushers_view`` return live
    dicts owned by ``MirrorLifecycleManager``;
    ``epoch_orchestrator`` owns the EpochManager registry.
    """
    ...
    def _schedule_on_main_loop(
        ...
        *,
        ...
        """Single chokepoint for cross-thread coroutine scheduling.

        RNS link callbacks fire from receiver threads where
        ``asyncio.ensure_future`` raises. This routes through
        ``run_coroutine_threadsafe`` with a done-callback so silent
        exceptions surface via ``logger.exception``; on a torn-down
        loop the coroutine is closed cleanly to avoid "never awaited"
        warnings.

        Every code path scheduling work onto ``self._loop`` from this
        orchestrator MUST go through this helper — direct
        ``run_coroutine_threadsafe`` / ``ensure_future`` calls are
        deprecated within this class.
        """
        loop = self._loop
