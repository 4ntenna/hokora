...
No channel-rotation grace window: per-message pushes authenticate the
pushing NODE via the Peer-table handshake and verify the LXMF sender's
Ed25519 signature. The channel's RNS identity is unused on this path,
so rotation doesn't open a signature-acceptance gap. Grace lives at the
announce-reception layer (channel_rotation_auth + peering).
"""
