...
def verify_sender_binding(
    ...
    Federation-receive write paths MUST call this before assigning a
    sequence number. A trusted-but-malicious peer could otherwise push a
    record with the victim's ``sender_hash`` and their own pubkey; the
    sig would verify but the row would land under the victim's identity.
    The structural fix recomputes the identity hash from the supplied
    64-byte RNS pubkey and rejects any mismatch — no TOFU surface,
    fail-closed before seq assign.
    ...
    Args:
        ...
        sender_rns_public_key: 64-byte ``X25519||Ed25519`` blob.
            ``None`` is accepted only when ``require_signed`` is False.
        lxmf_signed_part: bytes the LXMF sender signed.
        lxmf_signature: detached Ed25519 signature.
        require_signed: when True, missing pubkey or signature rejects.
    """
