        ...
        try:
            async with self._session_factory() as session:
                async with session.begin():
                    ...
                    # Loop prevention + outbound ban-content filter.
                    to_push = []
                    ...
                    # Attach full RNS pubkey (X25519||Ed25519) so the receiver
                    # can structurally bind sender_hash to its pubkey via
                    # truncated_hash equality — no TOFU, no path-cache fallback.
                    # Same mechanism as security.sealed.load_peer_rns_identity.
                    # Recall miss leaves the field None; receiver rejects
                    # when require_signed_federation is True.
                    for d in encoded:
