...
# Read-only seed listing for the TUI; mutation is CLI-only via ``hokora seed``.
...
# Internal sync_action for DeferredSyncItem rows holding live-push events;
# flushed in FIFO order on CDSP session resume.
