...
async def _resolve_channel_id(session, channel_ref: str) -> str:
    ...
    # 16-char hex matches the UUID-hex slice ``hokora channel create`` emits.
...
async def _rotate_channel_rns_key(channel_id):
    ...
    # In-memory grace-tracker is discarded on process exit; durable state
    # lives on the channel row, persisted below.
    mgr = KeyRotationManager()
    mgr.initiate_rotation(channel_id, old_identity, new_identity, old_destination)

    # Atomic rollover via .next sibling + timestamped backup.
