...
def init_cmd(data_dir, node_name, node_type, skip_luks_check, no_db_encrypt):
    ...
    if config_path.exists():
        ...
        if is_relay and not no_db_encrypt:
            # relays carry no community data, so encryption is opt-in
            db_encrypt = False
            click.echo("  Relay mode: database encryption disabled (no community data)")
        # Master key lives in a separate 0o600 file so hokora.toml is safe to back up.
    ...
    # API key for /api/metrics/ — atomic O_EXCL create at 0o600, idempotent.
