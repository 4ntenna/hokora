...
Filesystem-authored seed management. Authorization is unix file
permissions on ``hokora.toml`` and the RNS config directory; no
in-process auth.

Subcommands:

* ``hokora seed list`` — print currently-configured seeds.
* ``hokora seed add <name> <host[:port]>`` — add TCP or I2P seed.
* ``hokora seed remove <name>`` — remove a seed by section name.
* ``hokora seed apply [--restart]`` — operator-driven daemon restart;
  ``--restart`` SIGTERMs the running daemon and re-execs from
  ``hokorad.argv`` (dev-mode), otherwise prints the supervisor command.
"""
...
def _rns_config_dir() -> Optional[Path]:
    """Resolve the RNS config dir; falls back loudly to ``~/.reticulum``.

    Tolerates ``load_config()`` failures so ``hokora seed`` works on a
    bare install with no ``hokora.toml`` — the CLI operates on the RNS
    config file, not the database.
    """
