...
async def _assign_role(role_name, identity_hash, channel_id):
    ...
    # Init RNS so the CLI attaches as a shared-instance client of the
    # running daemon and inherits its path cache. Without this the CLI
    # has an empty cache and every sealed-key distribution defers — even
    # when the daemon has the recipient's identity live.
    import RNS
    ...
    if not getattr(RNS.Reticulum, "_hokora_cli_initialised", False):
        try:
            ...
            # Best-effort: deferred path surfaces an honest error if RNS init fails.
            pass
