...
async def handle_redeem_invite(
    ...
    # Identify-race fallback: read identity from the link directly.
...
async def handle_request_sealed_key(
    ...
    # Sealed-aware clients ask blindly per channel; null-blob reply means "nothing to persist".
    ...
    # Strict channel scope — node-scoped roles never satisfy this check.
...
async def handle_cdsp_session_init(
    ...
    # Resume: forward FIFO-ordered flushed items so the client can replay them.
...
async def handle_create_invite(
    ...
    # Append channel_id to the composite token so the TUI can connect directly.
