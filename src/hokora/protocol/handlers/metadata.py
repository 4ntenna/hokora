...
async def handle_node_meta(
    ...
    # ``show_private_channels`` lets non-members see private channel listings.
    ...
    # Single query for member_count + last_activity across all channels.
    ...
    for ch in channels:
        ...
        if requester_hash and ctx.permission_resolver:
            ...
            # Anonymous can't write to restricted/private/sealed channels.
    ...
    # node_identity_hash is the daemon's RNS hexhash, distinct from node_identity
    # (the node-owner admin hash); the TUI uses it to disambiguate same-named channels.
...
async def handle_member_list(
    ...
    # Private channels: membership; otherwise MANAGE_MEMBERS.
