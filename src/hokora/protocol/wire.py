...
def encode_sync_response(
    ...
    response = {
        "v": WIRE_VERSION,
        "nonce": nonce,
        "data": payload,  # field name kept for backward compat
    }
...
def encode_message_for_sync(msg) -> dict:
    """Encode a Message ORM object to a sync-response dict.

    ``sender_public_key`` (32-byte Ed25519) is filled by the sync handler;
    ``sender_rns_public_key`` (64-byte X25519||Ed25519) is filled by the
    federation pusher and consumed by ``verify_sender_binding``.
    """
    return {
