...
class LinkManager:
    ...
    def __init__(self, async_loop: asyncio.AbstractEventLoop):
        ...
        # Hook fires on Link close so the daemon can flush the zombie-link push buffer.
    ...
    def set_closed_hook(self, hook: Optional[Callable[[RNS.Link], None]]) -> None:
        """Register a callback fired from `_on_link_closed` with the dead Link."""
    ...
    def on_link_established(self, link: RNS.Link, channel_id: str):
        ...
        try:
            ...
            # 120s floor — RNS's RTT-derived keepalive is too tight on local links.
            try:
    ...
    def _on_link_closed(self, link: RNS.Link):
        ...
        # Flush any zombie-window push buffer; safe even if the link wasn't subscribed.
    ...
    def _on_identified(self, link: RNS.Link, identity: RNS.Identity):
        ...
        # Cache the LXMF delivery dest hash — different aspect from the hokora link.
        try:
    ...
    def _on_packet(self, message: bytes, packet: RNS.Packet):
        ...
        if is_epoch_frame(message):
            ...
            if frame_type == EPOCH_DATA:
                if em is None:
                    # Visible drop — surfaces handshake-wiring regressions.
        ...
        try:
            ...
            # Identify-race fallback: resolve identity directly from the link.
            if not requester_hash and link:
                try:
                    ...
                    if remote_id:
                        ...
                        # Cache lxmf.delivery dest hash too — different aspect.
                        try:
