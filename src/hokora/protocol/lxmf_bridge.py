...
class LXMFBridge:
    ...
    def get_any_router(self) -> Optional[LXMF.LXMRouter]:
        """Any available router for node-level outbound (prefers node_lxm_router)."""
    ...
    def _on_lxmf_delivery(self, message: LXMF.LXMessage) -> None:
        ...
        Production supplies a loop; the fallback paths cover tests that
        drive the bridge with or without a running loop.
        """
