...
``SyncContext`` carries every collaborator a handler might touch; to
stop handlers reaching for whatever happens to be on the bag, each
handler group annotates its ``ctx`` argument with a narrow
``typing.Protocol`` enumerating only the attributes it reads.
``SyncContext`` structurally satisfies all of them, so callers pass
the same concrete instance everywhere while mypy flags drift in any
handler whose reads exceed its declared Protocol.
"""
...
def populate_sender_pubkey(d: dict, sender_public_key: Optional[bytes]) -> None:
    """Single chokepoint to set 32-byte Ed25519 ``sender_public_key`` on a wire dict.

    No-op when ``sender_public_key`` is None — keeps the wire shape
    backwards-compatible with callers that don't have the pubkey at hand.
    """
...
def encode_message_for_wire(
    ...
    *,
    ...
    Single source of truth that respects the sealed-channel invariant.
    Subscribers advertising at-rest sealed support get ciphertext fields
    on the wire; legacy clients get server-side decrypted plaintext in
    ``body``. ``sealed_manager=None`` returns the row as-is.

    Membership is gated upstream; the RNS.Link transport is separately
    encrypted, so neither shape is visible to network observers. Legacy
    decrypt failure falls back to a stable marker so the wire dict
    remains well-formed.
    """
