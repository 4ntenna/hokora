...
Sender-binding invariant: callers MUST verify ``envelope.sender_hash``
against an Ed25519 key upstream. The role/permission/block-list checks
below all key on ``sender_hash`` and are meaningless against a
forgeable identity.
"""
...
class MessageProcessor:
    ...
    async def process_edit(self, session: AsyncSession, envelope: MessageEnvelope) -> Message:
        ...
        # Resolve channel once — sealed enforcement runs on both inserts.
        ...
        # Sealed channel without a key is a hard failure — no plaintext fallback.
    ...
    async def process_pin(
        ...
        # System message gets the same sealed treatment — it names the actor.
