...
Writes a msgpack heartbeat file every ``interval_s`` seconds, but only
when inline invariant checks pass — so a wedged announce or maintenance
loop cannot keep the heartbeat lying green. Atomic write via
``os.replace``; file is 0o644 and carries only the (already-public) node
identity hash and timestamp.

Consumed by ``ObservabilityListener`` (``/health/live`` mtime),
``systemd`` watchdog sidecars, and air-gapped LoRa operators.
"""
