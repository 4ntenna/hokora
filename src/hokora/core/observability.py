...
Routes ``/health/live``, ``/health/ready`` (unauth), and
``/api/metrics/`` (API-key gated via X-API-Key header or ``?key=``
query). Bind address is hard-coded to 127.0.0.1; non-loopback access
requires an external reverse proxy.

Threading: stdlib ``ThreadingHTTPServer`` spins a thread per request
(deviation from asyncio-everywhere). Handlers only read the heartbeat
file, the process-local API key, and marshal metrics to the daemon
loop via ``run_coroutine_threadsafe`` — no subsystem state is mutated
...
# surface reveals node identity + metrics fingerprint. Non-loopback
# access requires an external reverse proxy, never a TOML flag.
...
def _scrub_query_secrets(line: str) -> str:
    ...
    curl-from-loopback convenience; without scrubbing, a DEBUG log
    capture would persist the secret to disk.
    """
