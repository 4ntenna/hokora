...
class ChannelManager:
    ...
    async def announce_channels(self):
        """Announce all channels (hokora + LXMF delivery destinations).

        Every channel is announced for routing; visibility and access
        are gated downstream. Announces are staggered by
        ``announce_stagger_ms`` to work around an RNS 1.1.9 announce-cap
        eviction bug; set 0 to disable.
        """
        # Identity hash lets clients disambiguate same-named channels across nodes.
        node_identity_hash = None
        try:
            node_identity_hash = self.identity_manager.get_or_create_node_identity().hexhash
        except Exception:
            # Non-fatal: announce still works, clients just can't disambiguate.
                ...
                app_data = msgpack.packb(
                    {
                        ...
                        "destination_hash": dest_hash_hex,
                        "time": time.time(),
                        # Role hint surfaced by TUI Discovery; older TUIs ignore it.
