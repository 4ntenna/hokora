...
class HokoraDaemon:
    ...
    def __init__(self, config: NodeConfig):
        ...
        # Declarative teardown — subsystems register in start(), stop() runs in reverse.
    ...
    def _start_observability(self, role: str, node_identity) -> None:
        """Start HeartbeatWriter and ObservabilityListener.

        Relay role omits the ``maintenance_fresh`` invariant since relays
        have no maintenance loop.
        """
        node_identity_hash = getattr(node_identity, "hexhash", "unknown")

        def _rns_alive() -> bool:
            # ``interfaces`` lives on RNS.Transport, not the Reticulum instance.
            try:
                ifaces = list(getattr(RNS.Transport, "interfaces", []) or [])
                if not ifaces:
                    return True  # startup grace before transport subsystem registers
        ...
        if role == "community":
            ...
            def _maintenance_fresh() -> bool:
                if self._last_maintenance_run is None:
                    return True  # pre-first-tick grace
    ...
    async def stop(self):
        """Graceful shutdown via reverse-order ServiceRegistry teardown."""
    ...
    def _write_argv_file(self) -> None:
        """Persist argv + cwd + env for ``hokora seed apply --restart`` respawn."""
        try:
            payload = {
                "argv": list(sys.argv),
                "cwd": os.getcwd(),
