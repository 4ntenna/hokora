...
class AnnounceHandler:
    ...
    def parse_announce(app_data: bytes) -> Optional[dict]:
        ...
        Returns ``None`` for unknown types or malformed payloads.
        Key-rotation envelopes are returned with the inner payload
        normalised so callers see ``type=key_rotation`` consistently.
        """
        try:
        ...
        # Rotation envelope is {payload, old_signature, new_signature}; flatten so
        # callers see type=key_rotation without peeking into payload themselves.
