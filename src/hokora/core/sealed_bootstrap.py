...
class SealedKeyBootstrap:
    ...
    async def purge_plaintext_from_sealed_channels(self) -> int:
        """Retroactively enforce the sealed invariant.

        Two phases: (1) delete rows with no ciphertext (pre-invariant
        plaintext-only rows); (2) nullify plaintext on rows carrying
        both. The FTS5 ``messages_au`` update trigger only fires when
        ``new.body IS NOT NULL``, so the matching FTS rows are deleted
        explicitly to keep plaintext out of the search index.

        Returns the total row count affected.
        """
        total = 0
        try:
            async with self._session_factory() as session:
