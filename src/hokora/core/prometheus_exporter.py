...
Metric names and labels are a stable observability contract — treat
...
def _render_rnode_telemetry(iface: Any, name: str) -> list[str]:
    """Emit per-RNode LoRa telemetry as Prometheus gauges.

    Omit-rather-than-zero on missing fields — RNode firmware populates
    values asynchronously and a fake 0 would skew rate/threshold queries.
    Each field read is wrapped in try/except so one broken attribute
    only kills its own metric, not the rest of the scrape.
    """
    out: list[str] = []
    ...
    # bitrate is 0 until RNS computes it from radio config.
    try:
    ...
    # state=0/percent=0 means "no battery"; omit so mains-powered RNodes don't alarm.
    try:
...
async def render_metrics(
    ...
    Optional inputs are omitted entirely (not faked) when ``None``:
    ``rns_transport`` for per-interface metrics, ``mirror_manager`` for
    federation state, ``daemon_start_time`` falls back to module-import
    time. Opens one read-only session per call.

    Cross-thread callers must marshal via ``run_coroutine_threadsafe``.
    """
    lines: list[str] = []
