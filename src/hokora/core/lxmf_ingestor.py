...
class LxmfMessageIngestor:
    ...
    ``on_lxmf_delivery`` is the RNS-thread entry; ``ingest`` runs on the
    loop. EDIT/DELETE/REACTION/PIN are mapped to ``message_updated``.
    """
    ...
    def __init__(
        ...
        # Needed to decrypt sealed rows before emitting ``message_updated`` events.
                ...
                if self._live_manager:
                    ...
                    if envelope.type in (MSG_EDIT, MSG_DELETE, MSG_REACTION, MSG_PIN):
                        ...
                        if target_hash:
                            ...
                            if original:
                                # Decrypt sealed rows server-side; subscribers are membership-gated.
                        ...
                        # push_message decrypts sealed rows via its own sealed_manager.
