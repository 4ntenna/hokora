...
async def load_peer_rns_identity(identity_hash: str) -> "RNS.Identity":
    ...
    Single chokepoint for fetching a peer's full RNS public key (X25519 +
    Ed25519) when envelope-encrypting to them. RNS's ``known_destinations``
    cache is the canonical source; ``identities.public_key`` in our DB
    holds only the 32-byte Ed25519 half.

    On cache miss, issues ``Transport.request_path`` and polls
    ``Identity.recall`` for up to 3 s before raising
    :class:`SealedKeyDistributionDeferred` so the operator can retry once
    ...
    # Path request is async — response arrives via the next announce, so we poll.
    try:
...
async def distribute_sealed_key_to_identity(
    ...
    Single chokepoint for sealed-key distribution. ``node_identity`` may
    be passed by long-running callers to skip re-reading the identity
    file; when ``None``, the helper loads it from disk.
...
class SealedChannelManager:
    ...
    def encrypt(self, channel_id: str, plaintext: bytes) -> tuple[bytes, bytes, int]:
        ...
        aesgcm = AESGCM(key)
        nonce = os.urandom(12)
        # channel_id as AAD binds ciphertext to its channel: a row replayed
        # under a different channel_id fails authentication.
