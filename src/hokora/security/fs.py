...
def secure_client_dir(path: Path, recursive: bool = False) -> None:
    """Tighten a client-data directory to 0o700/0o600.

    With ``recursive=True`` walks the whole subtree (dirs 0o700, files
    0o600); symlinks are skipped. Idempotent; tolerates permission
    errors so a partial chmod won't trip startup.
    """
    path = Path(path)
    path.mkdir(parents=True, exist_ok=True)
    try:
